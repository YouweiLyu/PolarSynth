...
#############################################################
#### loading scenes from xml loading sensor parameters ####
#############################################################

save_parent_dir = './renderings/polar_scenes'
scene_file_dir = './scenes'
scene_files = [
    'sphere_materials_x.xml',
]
...
if __name__ == '__main__':
    ...
    scene_xml_path = os.path.join(scene_file_dir, scene_files[0])
    save_dir = os.path.join(save_parent_dir, scene_name)
    os.makedirs(save_dir, exist_ok=True)
    for idx in range(sensor_count):
        scene = mi.load_file(scene_xml_path, **{'y':-np.sin(np.deg2rad(phis[idx])),'z':-np.cos(np.deg2rad(phis[idx]))})
